...
void printMenu() {
	...
	std::cout << "Ввод номера устройства для безопасного извлечения" << std::endl << std::endl;
	std::cout << "Список USB устройств:" << std::endl << std::endl;
}
void printDevices()
{
...
LRESULT FAR PASCAL WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (message == WM_DEVICECHANGE)
	{
		...
		{
			system("CLS");
			std::cout << "Процесс безопасного извлечения ###" << std::endl;;
			printMenu();
			printDevices();
			break;
		}
	...
	while (true)
	{
		...
		if (ch >= '1' && ch <= '9')
		{
			if (ch - '0' <= Device::devices.size())
			{
				...
				{
					system("CLS");
					std::cout << "Устройство не извлечено" << std::endl;
