;Laba 6, variant 1
; (c) С.В. Зубков: Assembler. Язык неограниченных возможностей
; (c) 2012 evgeny3000 (некоторые куски кода)
; (c) 2012 Byteman (собственно мультизадачность с переключением по таймеру)
; BSUIR, VMSIS
...
old_mask2 db 0	
	...
	push	PM_seg
	pop	ds
;проверить на реальный режим (из болванки)
	mov	eax,cr0
	test	al,1
	jz	no_V86
...
no_windows:
	...
	cli
;считаем линейные адреса для дескрипторов сегментов
	xor	eax,eax
	mov	ax,RM_seg
	shl	eax,4
